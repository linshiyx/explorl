...
class PartialRollout(object):
    ...
    def extend(self, other_rollout):
        ...
        self.e_last_r = other_rollout.e_last_r
...
class SyncSampler(object):
    ...
    def __init__(self, env, policy, obs_filter,
        ...
        self.episode_rollout_provider = _episode_env_runner(
            self.env, self.policy, self.num_local_steps, self.horizon,
            self._obs_filter)
    ...
    def get_data(self):
        while True:
            ...
            if isinstance(item, PartialRollout):
                return item

    def get_episode_data(self):
        while True:
            item = next(self.episode_rollout_provider)
            return item
...
def _env_runner(env, policy, num_local_steps, horizon, obs_filter):
    ...
    while True:
        ...
        for _ in range(num_local_steps):
            action, pi_info = policy.e_compute(last_observation, *last_features)
        ...
        if not terminal_end:
            rollout.last_r, rollout.e_last_r = policy.value(last_observation, *last_features)

        # Once we have enough experience, yield it, and have the ThreadRunner
        # place it on a queue.
        yield rollout

def _episode_env_runner(env, policy, num_local_steps, horizon, obs_filter):
    while True:
        last_observation = obs_filter(env.reset())
        length = 0
        rewards = 0
        ...
        for _ in range(num_local_steps):
            action, pi_info = policy.compute(last_observation)
            observation, reward, terminal, info = env.step(action)
            observation = obs_filter(observation)
        ...
        if not terminal_end:
            rollout.last_r, rollout.e_last_r = policy.value(last_observation)

        yield rollout, rewards, length
